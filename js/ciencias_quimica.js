const preguntasQuimica = [
    {
        id: 1,
        pregunta: "¿Cuál es la masa molar del agua ($H_2O$), considerando $H=1$ g/mol y $O=16$ g/mol?",
        opciones: ["17 g/mol", "18 g/mol", "32 g/mol", "34 g/mol"],
        correcta: 1
    },
    {
        id: 2,
        pregunta: "En una reacción exotérmica, ¿qué sucede con la entalpía del sistema?",
        opciones: [
            "Se mantiene constante",
            "Aumenta ($\Delta H > 0$)",
            "Disminuye ($\Delta H < 0$)",
            "Es igual a la energía de activación"
        ],
        correcta: 2
    }
];
...
function renderizarPagina() {
    ...
    const preguntasVisibles = preguntasQuimica.slice(inicio, fin);
...
function actualizarInterfaz() {
    const totalPaginas = Math.ceil(preguntasQuimica.length / preguntasPorPagina);
...
if(finishBtn) {
    finishBtn.onclick = function() {
        if (!confirm("¿Deseas finalizar el ensayo de Química?")) return;
        examenFinalizado = true;
        let c = 0, i = 0, o = 0;
        preguntasQuimica.forEach(p => {
        ...
        document.getElementById('ratioDisplay').innerText = `${c} / ${preguntasQuimica.length}`;
        document.getElementById('percentageText').innerText = `${Math.round((c/preguntasQuimica.length)*100)}%`;
